...
try:
    ...
    print("  [expression] WARNING: deepface or opencv not available. Expression stream disabled.")
...
class ExpressionStream:
    ...
    def start(self):
        if not DEEPFACE_AVAILABLE:
            print("  [expression] DeepFace not available — skipping expression stream.")
            return
        self._running = True
    ...
    def _loop(self):
        ...
        # Force Windows to create the GUI window before the heavy AI starts
        cv2.namedWindow("Doctor Interview - Emotion Tracking", cv2.WINDOW_NORMAL)
        ...
        while self._running:
            ...
            # 1. ALWAYS DRAW THE FRAME FIRST (Keeps video smooth)
            cv2.putText(frame, f"Emotion: {last_label} ({int(last_conf*100)}%)", (20, 50), 
            ...
            # 2. ONLY RUN DEEPFACE EVERY 15 FRAMES (Prevents freezing)
            if frame_count % 15 == 0:
                try:
                    ...
                    last_conf  = result[0]["emotion"][last_label] / 100.0
